...
public class Player : MonoBehaviour
{
    ...
    public void ChangeHealth(float amount)
    {
        if (GM.Difficulty == GameManager.GameDifficulty.Versus)
        {
            switch (GM.vsEndurance)
            {
                case GameManager.VersusEndurance.Low :
                    health += amount;
                    health = Mathf.Clamp(health, 0, 50);
                    healthBar.UpdateBar(health / maxHealth);
                    healthText.text = ((50 - health) * 2) + " / " + 100;
                    break;
                case GameManager.VersusEndurance.Medium:
                    health += amount;
                    health = Mathf.Clamp(health, 0, 100);
                    healthBar.UpdateBar(health / maxHealth);
                    healthText.text = (100 - health) + " / " + 100;
                    break;
                case GameManager.VersusEndurance.High:
                    health += amount;
                    health = Mathf.Clamp(health, 0, 200);
                    healthBar.UpdateBar(health / maxHealth);
                    healthText.text = ((200 - health) / 2) + " / " + 100;
                    break;
            }
        }
        else
